...
class PostgresqlConnection:
    ...
    def __init__(self, database, create_db: bool = False):
        """
        ...
        self.database = ''.join(re.findall('\w', database))
        logging.info(self.database)
        self.ui = DatabaseInterface(self.database)
    ...
    def create_database(self):
        self.credentials[1] = 'postgres'
        self.cursor.handle_connection(self._create_database)
        self.credentials[1] = self.database
    ...
    def handle_connection(self, func, *args, **kwargs):
        ...
        while contador_time_out < time_out:
            try:
                ...
                logging.info("Error while connecting to Sqlite", error)
                time.sleep(1)
            finally:
                self.close()
            contador_time_out += 1
...
class DatabaseInterface():
    ...
    def _load_image(path_image, width: int = 50, height: int = 50):
        ...
        path = os.path.join(os.path.dirname(os.getcwd()), path_image)
...
class BotsDatabase:
    ...
    def __init__(self, database: str = None, create_db: bool = False):
        """
        ...
        """
        self.database = ''.join(re.findall('\w', database))
        self.pg = PostgresqlConnection(self.database, create_db)
        self.verify_credentials(self.pg.credentials, self.database)
        self.sqlite = SqliteConnection(self.database)
...
class TableBots(BotsDatabase):
    ...
    def __init__(self, database: str = 'Bots', create_db: bool = False):
        """
        Initialize the TableBots object.

        """
        super().__init__(database, create_db)
        self.bot_status = None
        self.bot_id = None
        self.bot_name = None
        self.bot_description = None
        self.bot_department = None
    ...
    def _insert_or_update(self):
        ...
        INSERT INTO bots (bot_id, bot_name, bot_description,bot_department,bot_status)
        VALUES ({self.bot_id}, '{self.bot_name}', '{self.bot_description}', {self.bot_department},{self.bot_status})
        ON CONFLICT (bot_id) 
    ...
    def generate_bot_id(self, bot_name):
        script = f"""
        select bot_id from bots
        where bot_name='{bot_name}'
        """
        ret = self.db.execute_script(script)
        logging.info(ret)
        if not ret:
            script = """
            select max(bot_id) from bots
            """
            ret = self.db.execute_script(script)
            logging.info(ret)
