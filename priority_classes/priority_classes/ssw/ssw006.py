...
class Ssw006(SswRequest):
    def __init__(self):
        super().__init__()

    def op006(self, *args, **kwargs):
        """
        ...
        response = self.session.post("https://sistema.ssw.inf.br/bin/ssw1602", headers=self.headers, data=data)
        ...
        # data = self.update_query_values(response.text, query_model, 'name', act=act, dummy=self.get_dummy(), **kwargs)
        # logging.info(data)
        # response = self.session.post("https://sistema.ssw.inf.br/bin/ssw1602", headers=self.headers, data=data)
        #
        # # showing results
