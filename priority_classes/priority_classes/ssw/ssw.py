...
class SswRequest:
    """
    The SswRequest class is used to manage and execute operations related to interacting
    with the SSW system, including initializing and managing the web browser, making requests,
    handling credentials, and managing timeouts.
    ...
    def __init__(self, domain: str = None, cpf: int = None, user: str = None, password: str = None, unit: str = None,
                 env: bool = True,prefix_env:str='SSW'):
        """
    ...
    def init_browser(self, profile_path: str = 'Default', ignore_profile: bool = True, path_to_downloads='downloads',
                     headless: bool = True, driver: str = 'edge', method: str = 'selenium',**kwargs):
        """
        ...
        self.method = method
        self.method = self._create_file_txt(self.method, 'config_method_browser', 'config', True)
    ...
    def run_async_method(self, headless, profile_path):
        """Function to run the async method"""
        self.event_loop = asyncio.new_event_loop()
        asyncio.set_event_loop(self.event_loop)
        self.event_loop.run_until_complete(self.pypperteer_browser(headless, profile_path))
        # loop.close()
    ...
    def chromedriver_install(self):
        try:
            ...
            self._create_folder('chromedriver')
    ...
    def firefox_driver(self, path_to_downloads, profile_path, ignore_profile, headless):
    ...
    async def pypperteer_browser(self, headless, profile_path):
        self.driver = await launch(headless=headless, userDataDir=profile_path)
    ...
    def _login_selenium(self):
        ...
        @time_out(2, delay=5,raise_exception=False)
        def try_login():
            # open the ssw link
    ...
    async def _enter_credentials_pypperteer(self):
        self.page = await self.driver.newPage()

        await self.page.goto(self.link)

        await self.page.evaluate(f'''() => {{
                document.querySelector("input[name='f1']").value = "{self.credentials[0]}";
            }}''')
        await self.page.evaluate(f'''() => {{
                document.querySelector("input[name='f2']").value = "{self.credentials[1]}";
            }}''')
        await self.page.evaluate(f'''() => {{
                document.querySelector("input[name='f3']").value = "{self.credentials[2]}";
            }}''')
        await self.page.evaluate(f'''() => {{
                document.querySelector("input[name='f4']").value = "{self.credentials[3]}";
            }}''')

        await self.page.click("a[id='5']")
        await asyncio.sleep(3)

        # maybe wait for some element that appears after login
        # await page.waitForSelector("div.logged-in")
    ...
    def _login_pyppeteer(self):
        # get credentials access
        # self._load_credentials()

        asyncio.get_event_loop().run_until_complete(self._enter_credentials_pypperteer())
        # self.event_loop.run_until_complete(self._enter_credentials_pypperteer())
    ...
    def execute_script_pypperteer(self, script, time_wait: int = 3):
        result = asyncio.get_event_loop().run_until_complete(self._execute_script_pypperteer(script, time_wait))
        # result = self.event_loop.run_until_complete(self._execute_script_pypperteer(script, time_wait))
        return result

    async def _post_data_pypper(self, url, data):
        # Set the headers
    ...
    def post_data_pypper(self, url, data):
        content = asyncio.get_event_loop().run_until_complete(self._post_data_pypper(url, data))
        logging.info(content)

    async def _get_data_pypper(self, url):
        # Set the headers
        await self.page.setExtraHTTPHeaders(self.headers)
    ...
    def get_data_pypper(self, url):
        content = asyncio.get_event_loop().run_until_complete(self._get_data_pypper(url))
        logging.info(content)
    ...
    def get_table(self, html, range_):
    ...
    def filter_df_by_values(self,df, filter_dict):
        ...
        query_str = ' & '.join(f"`{col}`.str.contains('{val}', case=False, na=False)" for col, val in filter_dict.items() if col in df.columns)
    ...
    def download_from_156(self, name_file: str = None):
        ...
        # write file
        filename = os.path.abspath(f"{self.path_to_downloads}/{name_file}") # Specify the filename to save the downloaded file
    ...
    async def _close_pypper(self):
        await self.driver.close()

    def close_pypper(self):
        asyncio.get_event_loop().run_until_complete(self._close_pypper())
        # self.event_loop.run_until_complete(self._close_pypper())
